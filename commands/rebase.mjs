...
async function resolveWithAI(file, content) {
  ...
  const aiPrompt = `You are resolving a git merge conflict. The file below contains conflict markers (<<<<<<< ======= >>>>>>>). Produce the final resolved file content — no conflict markers, no explanations, just the working code. Pick the best combination of both sides. If unsure, prefer the incoming (HEAD) changes.
